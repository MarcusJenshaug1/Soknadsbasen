...
function ProductDemoSkeleton() {
  return (
    ...
    >
      <div className="bg-[#eee9df] rounded-[24px] md:rounded-[32px] p-5 md:p-10">
        ...
        <div className="min-h-[420px] md:min-h-[480px] bg-white rounded-2xl border border-black/5 p-5 md:p-8">
          <div className="h-3 w-24 rounded-full bg-[#14110e]/10 mb-5" />
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
            {Array.from({ length: 4 }).map((_, i) => (
              <div
                key={i}
                className="aspect-[3/4] rounded-xl border border-black/10 bg-[#faf8f5]"
              />
            ))}
          </div>
        </div>
