      ...
      <div ref={ref} className="relative mb-3">
        <button
          ...
          className={cn(
            ...
            open
              ? "bg-[#eee9df] border-black/15"
              : "border-black/8 hover:bg-black/3 hover:border-black/15",
          )}
        >
          <div className="text-[10px] uppercase tracking-wider text-[#14110e]/45 mb-0.5">
            Aktiv søkerunde
          </div>
          <div className="font-medium text-[#14110e] truncate">
            {activeSession?.name ?? "Ingen aktiv sesjon"}
          </div>
          {startLabel && (
            <div className="text-[10px] text-[#14110e]/45 mt-0.5">siden {startLabel}</div>
          )}
        </button>

        {open && (
          <div className="absolute bottom-full left-0 right-0 mb-1 bg-[#faf8f5] border border-black/10 rounded-2xl shadow-lg overflow-hidden z-40">
            {activeSession && (
            ...
            {closedSessions.length > 0 && (
              <div className="px-4 py-3 border-b border-black/8 max-h-48 overflow-y-auto">
                <div className="text-[10px] uppercase tracking-wider text-[#14110e]/45 mb-2">Tidligere sesjoner</div>
                <div className="space-y-1">
                  {closedSessions.map((s) => (
                    <div key={s.id} className="flex items-center justify-between gap-2 px-3 py-2 rounded-xl hover:bg-black/3">
                      <div className="min-w-0">
                        <div className="text-[12px] font-medium truncate">{s.name}</div>
                        <div className="text-[10px] text-[#14110e]/45">
                      ...
                      <div className="flex items-center gap-1 shrink-0">
                        <Link
                          ...
                          className="px-2 py-1 rounded-lg text-[11px] border border-black/10 hover:border-black/25 transition-colors"
                        >
                          Se
                        </Link>
                        {!activeSession && (
            ...
            <div className="px-4 py-3">
              {!activeSession ? (
                ...
                <Link
                  href="/app/sesjoner"
                  onClick={() => setOpen(false)}
                  className="block text-center text-[11px] text-[#14110e]/55 hover:text-[#14110e] transition-colors"
                >
                  Se alle sesjoner
                </Link>
              )}
